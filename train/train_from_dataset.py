...
def main() -> None:
    ...
    ap.add_argument("--langs", default="en,pt")
    ...
    print(f"{'lang':5} {'n_train':>8} {'n_test':>7} | {'shipped_exact':>13} {'new_exact':>10} | "
          f"{'shipped_f1':>10} {'new_f1':>7}")
    for lang in args.langs.split(","):
        ...
        if os.path.exists(shipped_path):
            ...
            sh = evaluate(shipped, test)
        else:
            sh = {"exact": float("nan"), "f1": float("nan")}
        print(f"{lang:5} {len(train):8d} {len(test):7d} | {sh['exact']:13.3f} {new_scores['exact']:10.3f} | "
              f"{sh['f1']:10.3f} {new_scores['f1']:7.3f}")
